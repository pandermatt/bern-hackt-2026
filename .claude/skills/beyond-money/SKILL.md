...
- **The landing shows real screenshots, and they go stale.** `public/preview/*`
  is regenerated by `scripts/screenshots.mjs` against a running dev server. If
  you change how home, dashboard, budget or anomalies look, the landing is now
  lying until you re-run it. The heights in that script and `SHOT_ASPECT` in
  `components/landing.tsx` have to agree.
- **The footer is landing-only.** It renders from inside `<Landing />`, not the
  root layout, and takes no props — the reader there is always signed out. Do
  not put it back in the layout; inside the app it was a second navigation
  under every page.
